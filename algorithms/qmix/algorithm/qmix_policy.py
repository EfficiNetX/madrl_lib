...
class QMIXPolicy:  # QMIXPoliciesにしたほうがわかりやすい可能性がある
    ...
    def forward(self, obs, hidden_states, dones):
        """
        QMIXPolicyはforwardでRNNAgentのforwardのみ呼び出す（責務分離）
        """
        return self.agent_q_network.forward(obs, hidden_states, dones)
    ...
    def get_actions(
        ...
        """
        RNNAgentの出力（q_values, next_hidden_states）を受けて方針決定のみ担当
        """
        q_values, next_hidden_states = self.agent_q_network.forward(obs, hidden_states, dones)
        batch_size, n_agents, action_dim = q_values.shape
        if deterministic:
            actions = torch.argmax(q_values, dim=-1)
        else:
            random_numbers = torch.rand(batch_size, n_agents, device=q_values.device)
            random_actions = torch.randint(0, action_dim, (batch_size, n_agents), device=q_values.device)
